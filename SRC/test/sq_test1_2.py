...
def init(context):
    ...
    context.num = 8
    context.symbol = "AAA"
    context.model = test_group_model(context)

    schedule(schedule_func=algo, date_rule='1d', time_rule='09:31:00')


def test_group_model(context):
    model1 = GMModel()
    ...
    model2 = SVMModel(
        # AllSymbolSelector(list(index_list.keys())),
        # BlackFilter(),
        WhiteFilter(),
        TopNFilter(context.num),
    )
    # model2.train()
