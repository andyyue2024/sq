...
class SingleFactor1Order(Order):
    """
    SingleFactor1Order order target.
    ...
    def get_order_op(self, target: str) -> OrderOp:
        data = history_n(symbol=target, frequency="1d", count=35, end_time=self.now,
                         fields="open,high,low,close", fill_missing="last", adjust=ADJUST_PREV, df=True)
